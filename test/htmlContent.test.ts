...
// Media blocks (![](url)) are only emitted for standalone images, never for
// images inside a link or a converted data table cell (Telegram doesn't
// support media there) — and only because payload.ts no longer wraps the
// body in a "> " blockquote, which caused a production failure
// ("RICH_MESSAGE_PHOTO_NO_MEDIA_FOUND") when media blocks were nested there.
